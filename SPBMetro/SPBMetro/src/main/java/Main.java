...
public class Main
{
    ...
    private static void createStationIndex()
    {
        ...
        try
        {   //получили все данные (stations, connections, lines) из map.json в jsonData
            JSONParser parser = new JSONParser();
            JSONObject jsonData = (JSONObject) parser.parse(getJsonFile());

            //добавили ЛИНИИ в объект StationIndex (HashMap number2line)
            JSONArray linesArray = (JSONArray) jsonData.get("lines");
            parseLines(linesArray);

            //добавили СТАНЦИИ в объект StationIndex (TreeSet stations)
            JSONObject stationsObject = (JSONObject) jsonData.get("stations");
            parseStations(stationsObject);

            //добавили ПЕРЕСАДКИ в объект StationIndex (TreeMap connections)?
    ...
    // добавляет линии в объект StationIndex (в HashMap number2line)
